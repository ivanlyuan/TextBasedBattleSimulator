
Spell Effects:

	(quick): casting this spell does not end your turn
	
	NAME				EFFECT							COST
	Fireball			Deal 10 damage to an enemy target			7mp
	Meteor				Deal 20 damage to an enemy target			20mp
	Joker's neutral b	(quick) Deal 2 damage to an enemy target 			5mp
	Quick-Heal			(quick) Heal an ally target for 7 hp 			10mp

=============================================================================================

Remember to put levels.txt in the same directory as the .exe file.
Also, do not change anything in levels.txt, otherwise the game will not spawn enemies correctly.
I'll try do hide the data later, please forgive my laziness for now.
